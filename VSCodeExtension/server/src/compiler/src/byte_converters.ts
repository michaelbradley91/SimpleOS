...
{
    let non_negative_big_number = BigInt.asUintN(64, BigInt(big_number));
...
{
    let non_negative_big_number = BigInt.asUintN(16, BigInt(big_number));
...
{
    let non_negative_big_number = BigInt.asUintN(48, BigInt(big_number));
...
{
    const file_data = readFileSync(path).toString("hex");
    const byte_array = new Uint8Array(file_data.length / 2);
    for (let i = 0; i < file_data.length; i+=2)
    {
...
{
    const raw_bytes = new Uint8Array(Buffer.from(text, "utf-8"));
...
export function right_pad_bytes(bytes: Uint8Array, padding: number): Uint8Array
{
    ...
    const padding_length = padding - (bytes.length % padding);
    const padding_bytes = new Uint8Array(padding_length);
...
export function concat_bytes(byte_arrays: Uint8Array[])
{
    ...
    const merged_array = new Uint8Array(total_length);
    let offset = 0;
    byte_arrays.forEach(array => {
